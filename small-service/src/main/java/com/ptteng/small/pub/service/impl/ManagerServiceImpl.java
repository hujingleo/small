...
public class ManagerServiceImpl extends BaseDaoServiceImpl implements ManagerService {
	...
	public List<Long> getManagerIdsByName(String name, Integer start, Integer limit)
		...
		try {
			idList = dao.getIdList("getManagerIdsByName", new Object[] { name }, start, limit, false);
	...
	public List<Long> getManagerIdsByRoleID(Long roleID, Integer start, Integer limit)
		...
		if (log.isInfoEnabled()) {
			log.info(" get ids by roleID,start,limit  : " + roleID + " , " + start + " , " + limit);
		}
		...
		try {
			idList = dao.getIdList("getManagerIdsByRoleID", new Object[] { roleID }, start, limit, false);

		} catch (DaoException e) {
			log.error(" get ids  wrong by name,start,limit)  : " + roleID + " , " + start + " , " + limit);
			log.error(e);
			e.printStackTrace();
			throw new ServiceDaoException(e);
		}
	...
	public List<Long> getManagerIdsByRoleIDAndStatus(Long roleID, String status, Integer start, Integer limit)
		...
		if (log.isInfoEnabled()) {
			log.info(" get ids by roleID,status,start,limit  : " + roleID + " , " + status + " , " + start + " , "
					+ limit);
		}
		List<Long> idList = null;
		...
		try {
			idList = dao.getIdList("getManagerIdsByRoleIDAndStatus", new Object[] { roleID, status }, start, limit,
					false);

		} catch (DaoException e) {
			log.error(" get ids  wrong by name,start,limit)  : " + roleID + " , " + status + " , " + start + " , "
					+ limit);
			log.error(e);
			e.printStackTrace();
			throw new ServiceDaoException(e);
	...
	public List<Long> getManagerIdsByCondition(Map<String, Object> conditions, Integer start, Integer limit)
		...
		try {
			...
			for (String field : fieldSet) {
				if (field.contains("&")) {
					sql += field.split("&")[0] + " " + field.split("&")[1] + " '" + conditions.get(field) + "'";
					sql += " and ";
				} else {
